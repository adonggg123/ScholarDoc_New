...
function renderQueue() {
    ...
    container.innerHTML = idStudents.map((s, index) => {
        ...
        return `
            <div class="id-list-item ${isSelected ? 'selected' : ''}" style="padding: 12px; display: flex; align-items: center; gap: 16px; border-bottom: 1px solid var(--border-color);" onclick="selectIdStudent(${index})">
                ${avatarHtml}
...
function renderPanel() {
    ...
    container.innerHTML = `
        <div style="padding: 16px;">
            ...
            <button class="btn" style="width: 100%; background: var(--success); color: white; border: none; padding: 8px; border-radius: 10px; font-size: 12px; font-weight: 700; margin-bottom: 8px; cursor: pointer;" onclick="updateIdStatus('Verified')">Mark as Verified</button>
            <button class="btn btn-outline" style="width: 100%; border: 1.5px solid var(--error); color: var(--error); padding: 8px; border-radius: 10px; font-size: 12px; font-weight: 700; margin-bottom: 2px; cursor: pointer;" onclick="updateIdStatus('Missing')">Mark as Missing Documents</button>
            <button class="btn" style="width: 100%; background: transparent; color: var(--text-secondary); border: none; padding: 6px; font-size: 11px; font-weight: 700; cursor: pointer;" onclick="updateIdStatus('Rejected', true)">Permanent Rejection</button>
        </div>
    `;
